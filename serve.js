...
mongoose.connect(process.env.MONGODB_URI, { dbName: 'Weather-Journal' })
...
const registros = new mongoose.Schema({
    cidade: { type: String, required: true, trim: true, minlength: 2 },
    estado: { type: String, required: true, trim: true, maxlength: 2 },
    data_observacao: { type: String, required: true },
    temperatura_c: { type: Number, required: true },
    condicao: { type: String, required: true, trim: true, minlength: 2 },
    sensacao_termica: { type: Number, required: true },
    umidade_percentual: { type: Number, required: true },
    notas_usuario: { type: String, required: true, minlength: 2 }
...
app.patch('/registros/:id', async (req, res) => {
    try {
        const id = req.params.id;
        const novosDados = req.body;

        const registro = await Registros.findByIdAndUpdate(id, novosDados, { new: true });
...
app.delete('/registros/:id', async (req, res) => {
    try {
        ...
        const registro = await Registros.findByIdAndDelete(id);
